...
function Grille() {
    ...
    // LE PREMIER BADBUGS
    const delayedPosition = history.length >= 5 ? history[history.length - 5] : { row: 8, column: 8 };
    ...
    const [raisinRow, setRaisinRow] = useState(9)
                        ...
                        <div className="row" key={rowIndex}>
                            {row.map((column, columnIndex) => (
                                <div className="column" key={columnIndex}>
                                    ...
                                    {rowIndex === playerRow && columnIndex === playerColumn && <Codeman />}
                                    {rowIndex === delayedPosition.row && columnIndex === delayedPosition.column && <Badbugs />}
                                    {rowIndex === badbugsDeuxRow && columnIndex === badbugsDeuxColumn && <BadbugsDeux />}
                                    {rowIndex === pastequeRow && columnIndex === pastequeColumn && <Emoji icon={"🍉"} />}
                                    {rowIndex === citronRow && columnIndex === citronColumn && <Emoji icon={"🍋"} />}
                                    {rowIndex === raisinRow && columnIndex === raisinColumn && <Emoji icon={"🍇"} />}
                                    {rowIndex === kiwiRow && columnIndex === kiwiColumn && <Emoji icon={"🥝"} />}
